...
ntickets <- function(N,gamma,p) {
  ...
  n_nd=which.min(abs(1-gamma-pnorm(N+0.5, n1*p, sqrt(n1*p*(1-p)))))
  nd = n_nd+N

  plot(n1, 1-gamma-pnorm(N+0.5, n1*p, sqrt(n1*p*(1-p))),
  ...
  n_nc=which.min(abs(1-gamma-pnorm(N+0.5, n2*p, sqrt(n2*p*(1-p)))))
